...
exports.getSingalLog = getSingalLog;
exports.handleSignalLogDraw = handleSignalLogDraw;
exports.drawSignalLogImage = drawSignalLogImage;
...
for (const [key, value] of Object.entries(fontPaths))
    canvas_1.GlobalFonts.registerFromPath((0, path_1.join)('.', 'src', 'assets', value), key);
...
async function handleSignalLogDraw() {
    ...
    //     const imageBuffer = await drawSignalLogImage(tr, userLocale, signalResults[type], type);
    ...
    //         .addOptions(
    //           new StringSelectMenuOptionBuilder().setLabel(tr('CharacterPool')).setValue('character'),
    //           new StringSelectMenuOptionBuilder().setLabel(tr('WeaponPool')).setValue('weapon'),
    //           new StringSelectMenuOptionBuilder().setLabel(tr('RegularPool')).setValue('regular'),
    //           new StringSelectMenuOptionBuilder().setLabel(tr('BangbooPool')).setValue('bangboo'),
    //         ),
    //     );
    ...
    //         fetchReply: true,
    //       });
    //       handleSignalLogDraw(interaction, tr, userLocale, requestTime, signalResults, type);
    //       collector.stop();
    //     });
    //     collector.on('end', () => {
    //       resMessage.edit({
    ...
    //   drawInQueueReply(interaction, tr('DrawInQueue', { position: drawQueue.length - 1 }));
    // }
}
async function drawSignalLogImage() {
    // try {
    ...
    //   const [bg, characterImage, regularImage, booponImage, stampGoodLuckImage, stampBadLuckImage, yImage, ...restImages] = images;
    ...
    //   // Draw Signal Data
    //   drawRoundedRect(ctx, 70, isLimitedPool ? 100 : 125, 500, isLimitedPool ? 270 : 220, 30, boxColor); // (ctx, x, y, width, height, radius, color) - Increased height for new line
    //   const titleFontSize = 34;
    //   ctx.textAlign = 'left';
    //   drawText(ctx, tr('gacha_TotalCount'), selectedFont, 320, titleFontSize, titleFontSize - 4, 100, isLimitedPool ? 160 : 185); // (ctx, text, selectedFont, maxWidth, initialFontSize, minFontSize, x, y)
    //   drawText(ctx, tr('gacha_SRankCount'), selectedFont, 320, titleFontSize, titleFontSize - 4, 100, isLimitedPool ? 220 : 245); // (ctx, text, selectedFont, maxWidth, initialFontSize, minFontSize, x, y)
    //   drawText(ctx, tr('gacha_SRankAverage'), selectedFont, 320, titleFontSize, titleFontSize - 4, 100, isLimitedPool ? 280 : 305); // (ctx, text, selectedFont, maxWidth, initialFontSize, minFontSize, x, y)
    //   ctx.textAlign = 'right';
    //   drawText(ctx, `${signalResults.total}`, selectedFont, 100, titleFontSize, titleFontSize - 4, 540, isLimitedPool ? 160 : 185); // (ctx, text, selectedFont, maxWidth, initialFontSize, minFontSize, x, y)
    //   drawText(ctx, `${signalResults.data.length - 1 ?? 0}`, selectedFont, 100, titleFontSize, titleFontSize - 4, 540, isLimitedPool ? 220 : 245); // (ctx, text, selectedFont, maxWidth, initialFontSize, minFontSize, x, y)
    //   drawText(
    //     ctx,
    ...
    //   if (isLimitedPool) {
    //     drawText(ctx, `${isNaN(signalResults.limitedCharacterPullsAverage) ? 0 : signalResults.limitedCharacterPullsAverage}`, selectedFont, 100, titleFontSize, titleFontSize - 4, 540, 340);
    //     ctx.textAlign = 'left';
    //     drawText(ctx, tr('gacha_LimitedCharacterPulls'), selectedFont, 320, titleFontSize, titleFontSize - 4, 100, 340);
    //   }
    //   // Draw Signal Result
    ...
    //       if ((type === 'character' || type === 'weapon') && index !== 0 && standardCharacterIds.includes(data.id.toString())) {
    ...
    //         ctx.drawImage(standardCharacterImage, -overlaySize / 2, -overlaySize / 2, overlaySize, overlaySize);
    //         ctx.restore();
    //       }
    //       // Draw Agent Name
    //       if (index == 0) {
    //         ctx.textAlign = 'center';
    //         drawText(ctx, tr('gacha_Pity', { pity: signalResults.pity }), selectedFont, 160, 30, 28, 150 + offsetX, 585 + offsetY, '#808080'); // (ctx, text, selectedFont, maxWidth, initialFontSize, minFontSize, x, y)
    //       } else {
    ...
    //         const optimalFontSize = getOptimalFontSize(ctx, combinedText, selectedFont, maxTextWidth, initialFontSize, minFontSize);
    ...
    //   if (type === 'character' && signalResults.limitedCharacterPullsAverage !== undefined) {
    ...
    //   }
    //   // Draw Signal Type
    //   const i = type === 'character' ? 0 : type === 'weapon' ? 1 : type === 'regular' ? 2 : 3;
    ...
    //   drawRoundedRect(ctx, baseX - 60 + offsetX, 1440, 160, 160, 30, 'rgba(48, 48, 48, 255)');
    //   drawRoundedRect(ctx, baseX + 260 + offsetX, 1440, 160, 160, 30, 'rgba(48, 48, 48, 255)');
    //   // Draw Signal Type
    //   const iconWidth = 140;
    ...
    //   const y = 1450;
    //   ctx.drawImage(characterImage, startX + 0 * (iconWidth + iconGap), y, iconWidth, iconWidth);
    //   ctx.drawImage(characterImage, startX + 1 * (iconWidth + iconGap), y, iconWidth, iconWidth);
    //   ctx.drawImage(regularImage, startX + 2 * (iconWidth + iconGap), y, iconWidth, iconWidth);
    //   ctx.drawImage(booponImage, startX + 3 * (iconWidth + iconGap), y, iconWidth, iconWidth);
...
//   ctx.restore();
// }
// function getOptimalFontSize(ctx, text, selectedFont, maxWidth, initialFontSize, minFontSize) {
...
//   return fontSize;
// }
// function drawText(ctx, text, selectedFont, maxWidth, initialFontSize, minFontSize, x, y, color = 'white') {
//   let fontSize = initialFontSize;
//   ctx.font = `${fontSize}px ${selectedFont}`;
//   let textWidth = ctx.measureText(text).width;
//   while (textWidth > maxWidth && fontSize > minFontSize) {
