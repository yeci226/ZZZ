...
exports.default = {
    ...
    async execute(interaction, locale, ..._args) {
        ...
        if (interactionMember?.permissions instanceof discord_js_1.PermissionsBitField && !interactionMember?.permissions.has(discord_js_1.PermissionsBitField.Flags.ManageGuild)) {
            return interaction.reply({
                embeds: [new discord_js_1.EmbedBuilder().setColor('#E76161').setTitle(tr('admin_NoPermission')).setDescription(tr('admin_NoPermissionDesc'))],
...
const handleRemove = async (interaction, locale) => {
    const tr = (0, i18n_1.createTranslator)(locale);
    ...
    const data = await index_1.database.get(feature ?? '');
    const userData = data[selectedUser.id];
    if (!selectedUser.id) {
        return interaction.reply({
            embeds: [new discord_js_1.EmbedBuilder().setColor('#E76161').setTitle(tr('admin_RemoveFail')).setDescription(tr('admin_RemoveFailDesc'))],
            flags: discord_js_1.MessageFlags.Ephemeral,
        });
    }
        ...
        return interaction.reply({
            embeds: [
                new discord_js_1.EmbedBuilder()
                    .setColor('#E76161')
                    .setTitle(tr('admin_RemoveFail'))
                    .setDescription(tr('admin_RemoveFailUserOtherServer', {
    ...
    await index_1.database.delete(`${feature}.${selectedUser.id}`);
    return interaction.reply({
        embeds: [
            new discord_js_1.EmbedBuilder()
                .setColor('#F6F1F1')
                .setTitle(tr('admin_RemoveSuccess'))
                .setDescription(tr('admin_RemoveSuccessMessage', {
                user: `<@${selectedUser.id}>`,
...
const handleMove = async (interaction, locale) => {
    const tr = (0, i18n_1.createTranslator)(locale);
    const channel = interaction.options.getChannel('channel');
    const selectedFeature = interaction.options.getString('feature') ?? 'all';
    ...
    if (channel && clientMember && !clientMember.permissionsIn(channel).has(discord_js_1.PermissionsBitField.Flags.SendMessages)) {
        return interaction.reply({
            embeds: [
                new discord_js_1.EmbedBuilder()
                    .setColor('#E76161')
                    .setTitle(tr('admin_MoveFail'))
                    .setDescription(tr('admin_MoveNoPermission', {
                    channel: `<#${channel.id}>`,
                })),
            ],
            flags: discord_js_1.MessageFlags.Ephemeral,
    ...
    if ([discord_js_1.ChannelType.GuildText, discord_js_1.ChannelType.PrivateThread, discord_js_1.ChannelType.PublicThread, discord_js_1.ChannelType.GuildVoice].includes(channel?.type ?? 0)) {
        await interaction.deferReply({ flags: discord_js_1.MessageFlags.Ephemeral });
        const moveAutoDaily = async () => {
            let successCount = 0;
            const autoDailyData = (await index_1.database.get('autoDaily')) ?? {};
            const matchedUsers = Object.entries(autoDailyData)
                .filter(([, info]) => channelIdSet.has(info.channelId))
                .map(([userId]) => userId);
            for (const userId of matchedUsers) {
                ...
                if (userAutoDailyData) {
                    ...
                    await index_1.database.set(`autoDaily.${userId}`, userAutoDailyData);
                }
            }
            return successCount;
        };
        const moveAutoRedeem = async () => {
            let successCount = 0;
            const autoRedeemData = (await index_1.database.get('autoRedeem')) ?? {};
            ...
            for (const userId of matchedUsers) {
                ...
                if (userAutoRedeemData) {
                    ...
                    await index_1.database.set(`autoRedeem.${userId}`, userAutoRedeemData);
                    successCount++;
                }
            }
            return successCount;
        ...
        if (successCount === 0) {
            return interaction.editReply({
                embeds: [new discord_js_1.EmbedBuilder().setColor('#E76161').setTitle(tr('admin_MoveFail')).setDescription(tr('admin_MoveNoUser'))],
                flags: discord_js_1.MessageFlags.Ephemeral,
            });
        }
        return interaction.editReply({
            embeds: [
                new discord_js_1.EmbedBuilder()
                    ...
                    .setDescription(tr('admin_MoveSuccessMessage', {
