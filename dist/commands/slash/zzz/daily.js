...
exports.default = {
    ...
    async execute(interaction, locale, ..._args) {
        ...
        const hasAccount = await index_1.database.has(`${selectedUser.id}.account`);
        if (!hasAccount) {
            return interaction.editReply({
                embeds: [new discord_js_1.EmbedBuilder().setColor('#E76161').setTitle(tr('daily_NonAccount')).setDescription(tr('daily_NonAccountDesc'))],
            });
        }
        if (selectedAutoSign === 'off') {
            await index_1.database.delete(`autoDaily.${interactionUser.id}`);
            return interaction.editReply({
                embeds: [new discord_js_1.EmbedBuilder().setTitle(tr('autoDaily_Off')).setColor('#E76161')],
            });
        }
        else if (selectedTime || selectedTag || selectedAutoSign === 'on') {
            await index_1.database.set(`autoDaily.${interactionUser.id}`, {
                channelId,
                time: selectedTime,
                tag: selectedTag,
            });
            return interaction.editReply({
                embeds: [
                    new discord_js_1.EmbedBuilder()
                        ...
                        .setDescription(tr('autoDaily_Time', {
        ...
        const zzz = await (0, utilities_1.getUserZZZData)(interaction, locale, selectedUser.id);
        if (!zzz) {
            return interaction.editReply({
                embeds: [new discord_js_1.EmbedBuilder().setColor('#E76161').setTitle(tr('daily_Failed'))],
            });
        }
        ...
        if (res.code === -5003 || res.info.is_sign)
            return interaction.editReply({
                embeds: [new discord_js_1.EmbedBuilder().setColor('#E76161').setTitle(`${tr('daily_Failed')} ${tr('daily_Signed')}`)],
            });
        return interaction.editReply({
            embeds: [
                new discord_js_1.EmbedBuilder()
                    ...
                    .setDescription(`${tr('daily_Description', { a: `\`${todaySign?.name}x${todaySign?.cnt}\`` })}${info.month_last_day ? '' : `\n\n${tr('daily_DescriptionTmr', { b: `\`${tmrSign?.name}x${tmrSign?.cnt}\`` })}`}`)
                    .addFields({
