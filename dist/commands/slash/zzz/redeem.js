        ...
        switch (subcommand) {
            ...
            default:
                return interaction.reply({
                    embeds: [new discord_js_1.EmbedBuilder().setColor('#E76161').setTitle(tr('redeem_InvalidSubcommand'))],
                });
        }
    },
};
const handleList = async (interaction, locale) => {
    const tr = (0, i18n_1.createTranslator)(locale);
    const interactionUser = interaction.user;
    const selectedAccountIndex = parseInt(interaction.options.getString('account') || '0');
    const codes = await (0, utilities_1.getRedeemCodes)();
    const userUid = await (0, utilities_1.getUserUid)(interactionUser.id, selectedAccountIndex);
    const userRedeemedCodes = (await index_1.database.get(`${userUid}.redeemedCodes`)) || [];
...
const handleRedeemAll = async (interaction, locale) => {
    const tr = (0, i18n_1.createTranslator)(locale);
    const interactionUser = interaction.user;
    const selectedAccountIndex = parseInt(interaction.options.getString('account') || '0');
    ...
    if (!zzz)
        return interaction.editReply({
            embeds: [new discord_js_1.EmbedBuilder().setColor('#E76161').setTitle(tr('redeem_AccountNotFound'))],
            flags: discord_js_1.MessageFlags.Ephemeral,
        });
    const codes = await (0, utilities_1.getRedeemCodes)();
    const userUid = await (0, utilities_1.getUserUid)(selectedUser.id, selectedAccountIndex);
    const userRedeemedCodes = (await index_1.database.get(`${userUid}.redeemedCodes`)) || [];
    const noRedeemedCodes = codes.filter((code) => !userRedeemedCodes.includes(code.code));
    if (!noRedeemedCodes || noRedeemedCodes.length === 0) {
        const lastCookieRefresh = (await index_1.database.get(`${userUid}.lastCookieRefresh`)) || 0;
        const currentTime = Date.now();
        const oneDayInMs = 24 * 60 * 60 * 1000; // 24 hours
        if (currentTime - lastCookieRefresh >= oneDayInMs) {
            await (0, utilities_1.updateCookie)(selectedUser.id, selectedAccountIndex, zzz.cookie.toString());
            await index_1.database.set(`${userUid}.lastCookieRefresh`, currentTime.toString());
        }
        return interaction.editReply({
            embeds: [new discord_js_1.EmbedBuilder().setColor('#A2CDB0').setTitle(tr('redeem_NoCode'))],
            flags: discord_js_1.MessageFlags.Ephemeral,
        });
    }
    ...
    if (results.success.length + results.already.length + results.invalid.length + results.failed.length === 0) {
        return interaction.editReply({
            embeds: [new discord_js_1.EmbedBuilder().setColor((0, utilities_1.getRandomColor)()).setTitle(tr('redeem_NoCode'))],
            flags: discord_js_1.MessageFlags.Ephemeral,
        });
    }
    ...
    return interaction.editReply({
        embeds: [
            new discord_js_1.EmbedBuilder()
                ...
                .setDescription(results.success.map((code) => `✅ **${code.code}** (${code.message})`).join('\n') +
                (results.already.length ? '\n' + results.already.map((code) => `ℹ️ **${code.code}** (${code.message})`).join('\n') : '') +
                (results.invalid.length ? '\n' + results.invalid.map((code) => `⚠️ **${code.code}** (${code.message})`).join('\n') : '') +
                (results.failed.length ? '\n' + results.failed.map((code) => `❌ **${code.code}** (${code.message})`).join('\n') : '') +
...
const handleRedeem = async (interaction, locale) => {
    const tr = (0, i18n_1.createTranslator)(locale);
    const interactionUser = interaction.user;
    const selectedUser = interaction.options.getUser('user') || interactionUser;
    ...
    const userUid = await (0, utilities_1.getUserUid)(selectedUser.id, selectedAccountIndex);
    let userRedeemedCodes = (await index_1.database.get(`${userUid}.redeemedCodes`)) || [];
    try {
        const res = await zzz.redeem.claim(selectedCode);
        if (res.retcode == 0 || res.message == 'OK') {
            if (!userRedeemedCodes.includes(selectedCode))
                userRedeemedCodes.push(selectedCode);
            userRedeemedCodes = Array.from(new Set(userRedeemedCodes));
            await index_1.database.set(`${userUid}.redeemedCodes`, userRedeemedCodes);
            return interaction.editReply({
                embeds: [new discord_js_1.EmbedBuilder().setColor((0, utilities_1.getRandomColor)()).setTitle(tr('redeem_Success')).setThumbnail('https://static.wikia.nocookie.net/zenless-zone-zero/images/4/4c/Item_Polychrome.png')],
                flags: discord_js_1.MessageFlags.Ephemeral,
            });
        }
            ...
            userRedeemedCodes = Array.from(new Set(userRedeemedCodes));
            await index_1.database.set(`${userUid}.redeemedCodes`, userRedeemedCodes);
            return (0, utilities_1.failedReply)(interaction, res.message);
        }
        else {
            const userAccount = await index_1.database.get(`${selectedUser.id}.account`);
            ...
            if (userCookie.cookie.includes('cookie_token_v2') || userCookie.cookie.includes('account_mid_v2')) {
                return (0, utilities_1.failedReply)(interaction, tr('redeem_CookieTokenInvalid'));
            }
            else {
                return (0, utilities_1.failedReply)(interaction, tr('redeem_NoCookie'));
            }
        }
    }
    catch (e) {
        return (0, utilities_1.failedReply)(interaction, e.message);
    }
};
const handleAutoRedeem = async (interaction, locale) => {
    const tr = (0, i18n_1.createTranslator)(locale);
    ...
    const zzz = await (0, utilities_1.getUserZZZData)(interaction, locale, interaction.user.id);
    if (!zzz)
        return interaction.editReply({
            embeds: [new discord_js_1.EmbedBuilder().setColor('#E76161').setTitle(tr('redeem_AccountNotFound'))],
            flags: discord_js_1.MessageFlags.Ephemeral,
        });
    const userAccount = await index_1.database.get(`${interactionUser.id}.account`);
    if (!userAccount[0].cookie.includes('cookie_token_v2') && !userAccount[0].cookie.includes('account_mid_v2')) {
        return (0, utilities_1.failedReply)(interaction, tr('redeem_NoCookie'));
    }
    if (selectedEnable === 'on') {
        await index_1.database.set(`autoRedeem.${interactionUser.id}`, {
            channelId,
        ...
        return interaction.editReply({
            embeds: [
                new discord_js_1.EmbedBuilder()
                    ...
                    .setDescription(tr('autoRedeem_Tag', {
        ...
        await index_1.database.delete(`autoRedeem.${interactionUser.id}`);
        return interaction.editReply({
            embeds: [new discord_js_1.EmbedBuilder().setColor('#E76161').setColor('#E76161').setTitle(tr('autoDaily_Off'))],
            flags: discord_js_1.MessageFlags.Ephemeral,
        });
    }
};
const handleRedeemResult = async (code, res, redeemedCodes, userUid, locale) => {
    const tr = (0, i18n_1.createTranslator)(locale);
    let status = 'failed';
    let message = '';
    switch (res.retcode) {
    ...
    if (status !== 'failed' && !redeemedCodes.includes(code)) {
        ...
        await index_1.database.set(`${userUid}.redeemedCodes`, Array.from(new Set(redeemedCodes)));
    }
    return { status, message };
};
function createProgressEmbed(codes, currentIndex, locale) {
    const tr = (0, i18n_1.createTranslator)(locale);
    ...
    return new discord_js_1.EmbedBuilder()
        ...
        .setDescription(tr('redeem_ProcessingDesc', { noRedeemedCodes: (codes.length - currentIndex).toString(), seconds: ((codes.length - currentIndex) * 3).toString() }) +
        '\n\n' +
        (processedResults ? `${tr('redeem_Processed')}:\n${processedResults}` : ''))
        .setThumbnail('https://cdn.discordapp.com/attachments/1231256542419095623/1361321499549499432/bqqinrjkvtsd1.gif?ex=67fe54f1&is=67fd0371&hm=286f61395cfec0fa862d54c58dbc7b5b6aa20f89f76fd28756ca2cca0c7058aa&');
}
