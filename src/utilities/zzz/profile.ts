...
export async function drawMainImage(tr: (key: string, args?: any) => string, userLocale: string, userData: any, record: any) {
  try {
...
export async function drawCharacterImage(
  ...
  try {
    ...
    for (let i = 0; i < 6; i++) {
      if (!character.equip.some((equip: any) => equip.equipment_type - 1 === i)) {
        character.equip.push({ equipment_type: i + 1 });
      }
    }
    character.equip.sort((a: any, b: any) => a.equipment_type - b.equipment_type);
    ...
    if (character.skin_list.length > 0 && characterData && characterData.skin) {
      ...
      if (skin) {
        ...
        const skinImage = characterData && characterData.skin ? characterData.skin[String(skin.skin_id)] : null;
        if (skinImage)
    ...
    const imagePaths = [
      ...
      ...character.skills.map(
        (skill: any) => `./src/assets/images/icons/skills/${skill.skill_type}.png`
      ),
    ...
    const baseOffset = (offsetCharacter as any)[characterId] || {};
    const skinOffset = skinId && (offsetCharacterSkin as any)[characterId] ? (offsetCharacterSkin as any)[characterId][skinId] : null;
    ...
    character.equip.map((equip: any, index: number) => {
      ...
      equip.main_properties?.forEach((prop: any) => {
        const image = propertyImageMap[propertiesId[prop.property_id as keyof typeof propertiesId]];
      ...
      equip.properties?.forEach((prop: any, index: number) => {
    ...
    if (character.weapon) {
      ...
      ctx.fillStyle = "#FFFFFF29";
      ctx.textAlign = "center";
      ctx.fillText(`EMPTY`, 1320 + 650 / 2, 620 + 240 / 2, 650 / 2);
    }
...
function drawTextWithBackground(ctx: SKRSContext2D, text: string, x: number, y: number, options: any = {}) {
  // 解構選項參數，提供預設值
  const {
    font = "20px Arial",
    textColor = "black",
...
function drawPaintSplatter(ctx: SKRSContext2D, canvasWidth: number, canvasHeight: number, options: any = {}) {
  const {
