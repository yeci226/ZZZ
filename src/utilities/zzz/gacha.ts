...
async function getAgentImageUrl(agent: Agent) {
  if (!agent?.id) return "./src/assets/images/icons/gacha/none.png";
  const agentId = parseInt(agent.id);

  if (agentId < 10000) {
    return getAvatarUrl(agentId.toString());
  } else if (agentId >= 10000 && agentId < 50000) {
    const weaponData = await getWeaponData(agentId.toString());
    return weaponData?.iconUrl || "./src/assets/images/icons/gacha/none.png";
  } else {
    const bangbooData = await getBangbooData(agentId.toString());
    return bangbooData?.iconUrl || "./src/assets/images/icons/gacha/none.png";
  }
}
...
export async function getSingalLog(interaction: ChatInputCommandInteraction, tr: (key: string, args?: any) => string, userLocale: string, input: Record<string, string>) {
  ...
  interface SignalData {
    ...
    pity: number;
    data: any[];
    limitedCharacterPullsAverage?: number;
  }

  const allSignalSRanklist: Record<string, SignalData> = {
...
export async function drawSignalLogImage(tr: (key: string, args?: any) => string, userLocale: string, signalResults: any, type: string) {
  try {
    ...
    // Draw Signal Result
    signalResults.data.forEach((data: any, index: number) => {
