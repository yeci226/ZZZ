...
import { getUserLang, getRandomColor } from '@/utilities';
...
class AutoDailySignSystem {
  ...
  formatResult(locale: LanguageEnum, result: { status: string; info: any; reward: any; rewards: any; uid: number | null }, userId: string) {
  ...
  async processAccount(locale: LanguageEnum, account: Account, userId: string, channelId: string, tag: string) {
  ...
  updateStatistics(nowTime: string, duration: number, averageTime: number) {
    ...
    new Logger('自動簽到').info(`成功簽到: ${this.stats.success} 個`);
    new Logger('自動簽到').info(`已簽到過: ${this.stats.alreadySigned} 個`);
    new Logger('自動簽到').info(`簽到失敗: ${this.stats.failed} 個`);
...
export default async function autoDailySign() {
  try {
    ...
    if (!dailyData) {
      new Logger('自動簽到').info('沒有找到需要自動簽到的用戶數據');
    } else {
      for (const userId of Object.keys(dailyData)) {
        const scheduledTime = dailyData[userId]?.time || '13';
        if (parseInt(scheduledTime) !== parseInt(currentHour)) return;
