...
import { drawInQueueReply, getUserLang } from "../utilities.js";
import { toI18nLang, createTranslator } from "../core/i18n.js";
...
export async function renderPage(
  ...
  tr,
  userLocale,
  userState = getUserState(interaction.user.id)
) {
  try {
    ...
    if (
      userState.canvasCache.has(`post_${currentcategory_index}_${currentPage}`)
    ) {
      console.log(
        `正在使用 ${interaction.user.displayName} 第 ${currentPage + 1} 頁的快取圖片 (類別: ${categoriesTranslate[categories[currentcategory_index]]})`
      );
      return userState.canvasCache.get(
        `post_${currentcategory_index}_${currentPage}`
      );
    } else {
      console.log(
        `正在渲染 ${interaction.user.displayName} 第 ${currentPage + 1} 頁的圖片 (類別: ${categoriesTranslate[categories[currentcategory_index]]})`
      );
    }
    ...
    const renderPostPage = async () => {
      try {
        ...
        if (userState.canvasCache.has(`post_${currentcategory_index}`)) {
          console.log(
            `正在使用 Posts 快取圖片 (類別: ${categoriesTranslate[categories[currentcategory_index]]})`
          );
          return userState.canvasCache.get(`post_${currentcategory_index}`);
        } else {
          console.log(
            `正在渲染 Posts 圖片 (類別: ${categoriesTranslate[categories[currentcategory_index]]})`
          );
        }

        // Load post
        const posts = (await db.get("posts")).filter(
          (post) =>
            categories[currentcategory_index] === "general" ||
            post.post_category === categories[currentcategory_index]
        );
        const post_images = await Promise.all(
          posts.map((post) =>
            loadImageAsync(
              post.post_image ||
                "./src/assets/images/interknot/postDefault.webp"
            )
          )
        );
        const post_author_avatars = await Promise.all(
          posts.map((post) => loadImageAsync(post.post_author_avatar))
        );
        ...
        for (; last_post < posts.length; last_post++) {
          ...
          let shortestColumnIndex = column_heights.indexOf(
            Math.min(...column_heights)
          );
          ...
          const image_height = Math.floor(
            (image.naturalHeight * post_width) / image.naturalWidth
          );
          ...
          function sliceStr(str, maxWidth, lines, fontSize) {
            ...
            if (slicedStrLines.length > lines) {
              slicedStrLines[lines - 1] =
                slicedStrLines[lines - 1].slice(0, -1) + "...";
            }
          ...
          titleLines.forEach((line, index) => {
            postCtx.fillText(
              line,
              next_x + 20,
              next_y + post_height - 10 - 40 * (titleLines.length - index)
            );
          });
          ...
          drawRoundedRect(
            postCtx,
            next_x + 16,
            next_y + post_height - 20 - 40 * titleLines.length - 94,
            78,
            78,
            39,
            {
          ...
          drawRoundedRect(
            postCtx,
            next_x + 20,
            next_y + post_height - 20 - 40 * titleLines.length - 90,
            70,
            70,
            35,
            {
          ...
          postCtx.fillText(
            author,
            next_x + 20 + 40 + 40,
            next_y + post_height - 20 - 40 * titleLines.length - 35
          );
          ...
          postCtx.moveTo(
            next_x + 20 + 40 + 40,
            next_y + post_height - 20 - 40 * titleLines.length - 23
          );
          postCtx.lineTo(
            next_x + post_width - 20,
            next_y + post_height - 20 - 40 * titleLines.length - 23
          );
          postCtx.stroke();
          ...
          postCtx.font = `24px ${selectedFont}`;
          postCtx.fillText(
            `${post.post_viewed_times}`,
            next_x + 20 + viewEmojiWidth,
            next_y + 40
          );
        }
        ...
        totalPages = Math.min(
          Math.ceil(heighestColumn_height / page_height),
          100
        );
    ...
    userState.canvasCache.set(
      `post_${userState.category}_${userState.page}`,
      imageBuffer
    );
    return imageBuffer;
  } catch (e) {
