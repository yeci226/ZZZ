...
const STATUS_CONFIG = {
  success: { icon: "✅", color: "#6ee09a", bg: "rgba(80,200,120,0.13)", border: "rgba(100,220,140,0.40)" },
...
export async function buildZZZRedeemCard(
  ...
  let totalCodes = 0;
  let accountCount = payload.accounts.length;
  ...
  ctx.fill();
  ctx.strokeStyle = "rgba(255, 195, 50, 0.22)";
  ctx.lineWidth = 1.8;
  ctx.stroke();
  ...
  ctx.fillText("禮包碼兌換結果", px + 24, py + 46);
  ...
  ctx.strokeStyle = "rgba(255, 195, 50, 0.20)";
  ctx.lineWidth = 1;
  ctx.beginPath();
  ...
  for (const acc of payload.accounts) {
    ...
    ctx.fillText(`${acc.nickname}  (${acc.uid})`, px + 24, curY + 34);
    ...
    for (const codeResult of acc.codes) {
      ...
      if (codeResult.rewards?.length) {
        const rewardsText = codeResult.rewards.join("  •  ");
        ctx.fillStyle = "#a09060";
        ctx.font = `17px ${font}`;
        // Truncate if too long
        let displayText = rewardsText;
        const maxW = pw - 280;
