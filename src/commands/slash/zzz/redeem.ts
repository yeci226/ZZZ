...
export default {
    ...
    async execute(client: Client, interaction: ChatInputCommandInteraction, args: any[], tr: any, db: QuickDB) {
        ...
        if (subcommand == "list") {
            ...
            const codes = await getRedeemCodes();
            let userRedeemedCodes: string[] = (await db.get(`${uid}.redeemedCodes`)) || [];
            const noRedeemedCodes = codes.filter(
                (code: any) => !userRedeemedCodes.includes(code.code)
            );

            // 檢查是否需要更新Cookie（無論是否有未兌換的禮包碼）
            const lastCookieRefresh: number = (await db.get(`${uid}.lastCookieRefresh`)) || 0;
            ...
            if (userRedeemedCodes.length > 0) {
                await db.set(`${uid}.redeemedCodes`, Array.from(new Set(userRedeemedCodes)));
            }
                    ...
                    new EmbedBuilder()
                        ...
                        .setDescription(
                            ...
                            (results.already.length
                                ? "\n" +
                                results.already
                                    .map((code: any) => `ℹ️ **${code.code}** (${code.message})`)
                                    .join("\n")
                                : "") +
                            (results.invalid.length
                                ? "\n" +
                                results.invalid
                                    .map((code: any) => `⚠️ **${code.code}** (${code.message})`)
                                    .join("\n")
                                : "") +
                            (results.failed.length
                                ? "\n" +
...
async function handleRedeemResult(code: string, res: any, userRedeemedCodes: any[], tr: any) {
    let status = "failed";
    let message = "";
