...
export default {
    name: "detail",
    execute: async (client: Client, message: Message, args: string[], emoji: any) => {
        const db = client.db;
        const id = args[0];
        ...
        message.reply({
            embeds: [
                new EmbedBuilder()
                    ...
                    .addFields(
                        { name: "自動簽到", value: `${daily ? daily?.time : "未開啟"}`, inline: false },
                        { name: "自動通知", value: `${redeem ? "已開啟" : "未開啟"}`, inline: false },
                        ...accountFields
                    ),
            ],
        });
