...
import { Events, EmbedBuilder, WebhookClient, ChannelType, MessageFlags, BaseInteraction, ChatInputCommandInteraction, ButtonInteraction, ContextMenuCommandInteraction } from "discord.js";
...
client.on(Events.InteractionCreate, async (interaction: BaseInteraction) => {
  ...
  if (interaction.isCommand()) {
    ...
    for (let option of (interaction as ChatInputCommandInteraction).options.data) {
    ...
    try {
      ...
      (command as any).execute(client, chatInteraction, args, i18n, db, emoji);
      const time = `花費 ${(
    ...
    if (!command) return;
    try {
      (command as any).execute(client, interaction as ContextMenuCommandInteraction);
    } catch (e: any) {
      new Logger("指令").error(`錯誤訊息：${e.message}`);
      await interaction.reply({
        content: "哦喲，好像出了一點小問題，請重試",
